...
describe('Registry tests', () => {
    ...
    it('Get resource by different locations', () => {
        const loc0 = RegistryUtil.createLoc('sapphire')
        const loc1 = RegistryUtil.createLoc('sapphire')

        const registry = RegistryUtil.createRegistry<string>('gem_color')
        registry.register(loc0, 'blue')

        expect(registry.getRef(loc1).getResource()).toBe('blue')
    })
})